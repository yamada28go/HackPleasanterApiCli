    ...
    public class Exporter
    {
        ...
        public void DoExporter(DirectoryInfo workDir, DefinitionExtractorConfig config)
        {
            ...
            if (true == config.Output.UseDescriptionAsVariableName)
            {
                ...
                {
                    // サイトIDでまとめる
                    var groupedSite = r.InterfaceDefinitionConverter.Select(x => Tuple.Create(x.SiteId, x)).GroupBy(x => x.Item1);
                    foreach (var siteItem in groupedSite)
                    {
                        // サイト内のカラム名一覧を作成
                        var xl = siteItem.Select(x => x.Item2).ToList();
                        var gl = xl.Select(x => Tuple.Create(x.VariableName, x)).GroupBy(x => x.Item1).ToList();
                        // 名前が重複しているパターン
